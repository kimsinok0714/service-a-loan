...
def check_numerical_drift(train_df, pred_df, columns, threshold=20):
    ...
    for col in columns:
        ...
        # 평균 차이를 %로 계산
        # 학습 데이터 평균과 운영 데이터 평균 간의 변화율을 백분율로 계산
        if train_mean != 0:
...
def main():
    ...
    if total_drifted >= 3 or pred_diff > 10:
        print("\n  ** 재학습이 필요합니다! **")
        print("  → retrain.py를 실행하여 모델을 재학습하세요.")
        return True
    else:
        print("\n  현재 모델이 안정적입니다.")
        return False
